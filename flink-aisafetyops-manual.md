# Flink для AISafetyOps: подробный manual по внедрению и эксплуатации
...
Этот документ адаптирует базовую архитектуру Flink под банковский кейс **AISafetyOps**: потоковую обработку логов LLM-агентов, событий guardrail-детекторов и инцидентов безопасности.
...
Для AISafetyOps Flink подходит хорошо, если задача выглядит так:
...
## 5. Целевая бизнес-модель данных для AISafetyOps
...
Это один из ключевых паттернов для AISafetyOps.
...
В AISafetyOps вас интересует реальный порядок событий безопасности, а не момент, когда worker их увидел.
...
### 10.2 Join strategy для AISafetyOps
...
Для production AISafetyOps чаще лучше **application mode** или operator-managed application deployment, потому что:
...
metadata:
  name: aisafetyops-guardrails
spec:
  image: registry.bank.local/flink/aisafetyops-guardrails:1.0.0
  flinkVersion: v2_1
  serviceAccount: flink
  flinkConfiguration:
    taskmanager.numberOfTaskSlots: "2"
    state.checkpoints.dir: s3://bank-flink-prod/checkpoints/aisafetyops
    state.savepoints.dir: s3://bank-flink-prod/savepoints/aisafetyops
  ...
  taskManager:
    resource:
      ...
      memory: "8192m"
  job:
    jarURI: local:///opt/flink/usrlib/aisafetyops-guardrails.jar
    parallelism: 8
...
Для AISafetyOps эти пять слоев обычно соответствуют такому потоку:
...
env.execute("aisafetyops-guardrails");
```
